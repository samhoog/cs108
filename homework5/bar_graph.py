...
class BarGraph():
    ...
    def __init__(self, data, color='random'):
        ...
        for num in self.data:
            if num < 0: 
                # If there is a negative value, raise a value error
                raise ValueError("You cannot have a negative value")
        if self.data == []:
            # If the list is empty, raise a value error 
            raise ValueError("You cannot have an empty data list")
    def __str__(self):
        """Return a string printing the color and list of data"""
        return("Bar Graph - Color: " + self.color + " Data: " + str(self.data))
    
    def draw(self, canvas, total_width=500, total_height=500):
        """Draw the bar graph using the list of integers"""
        # Create and pack a canvas with the user given width and height
        self.canvas = tkinter.Canvas(tkinter.Tk(), width=total_width,
...
if __name__ == "__main__":
    ...
    #negative_data = BarGraph([5, 2, -5, 10])